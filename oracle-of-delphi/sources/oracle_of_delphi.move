...
module overmind::price_oracle {
    ...
    */
    fun init_module(admin: &signer) {
        ...
        move_to(&resource_signer, PriceBoard {
            prices: table::new<String, PriceFeed>()
        });

        move_to(&resource_signer, State {
            signer_cap,
            price_feed_updated_event: account::new_event_handle<PriceFeedUpdatedEvent>(admin)
        });
    }
    ...
    */
    public entry fun update_price_feed(
        ...
        let resource_address = get_resource_address(&signer::address_of(admin));
        assert!(exists<State>(resource_address), ErrorCodeForAllAborts);

        let state: &mut State = borrow_global_mut<State>(resource_address);
        let prices: &mut Table<String, PriceFeed> = &mut borrow_global_mut<PriceBoard>(resource_address).prices;

        let new_price_feed: PriceFeed = PriceFeed{
        ...
        let price_feed: &mut PriceFeed = table::borrow_mut_with_default<String, PriceFeed>(prices, pair, new_price_feed);
        *price_feed = new_price_feed;

        event::emit_event<PriceFeedUpdatedEvent>(&mut state.price_feed_updated_event, PriceFeedUpdatedEvent{
            pair,
            price: new_price_feed.price,
    ...
    */
    public fun get_price(pair: String): Price acquires PriceBoard {
        get_price_no_older_than(pair, 10_800)
    }
    ...
    */
    ...
    acquires PriceBoard {
        let (price, timestamp) = get_price_and_timestamp(pair);
        assert!(timestamp::now_seconds() - timestamp < maximum_age_seconds, ErrorCodeForAllAborts);
        price
    }
    ...
    */
    public fun get_price_unsafe(pair: String): (Price, u64) acquires PriceBoard {
        get_price_and_timestamp(pair)
    }

    //==============================================================================================
    // Helper functions
    //==============================================================================================
    fun get_price_and_timestamp(pair: String): (Price, u64) acquires PriceBoard{
        let resource_address = get_resource_address(&@overmind);

        let prices: &Table<String, PriceFeed> = &borrow_global<PriceBoard>(resource_address).prices;
        assert!(table::contains<String, PriceFeed>(prices, pair), ErrorCodeForAllAborts);

        let price_feed: &PriceFeed = table::borrow<String, PriceFeed>(prices, pair);

        (*&price_feed.price, *&price_feed.latest_attestation_timestamp_seconds)
    }

    fun get_resource_address(source: &address): address {
