...
module overmind::streams {
    ...
    */
	public fun create_stream<PaymentCoin>(
        ...
        let coin_value: u64 = coin::value<PaymentCoin>(&payment);

        assert!(receiver != sender, ESenderCannotBeReceiver);
        assert!(coin_value > 0, EPaymentMustBeGreaterThanZero);
        assert!(duration_in_seconds > 0, EDurationMustBeGreaterThanZero);

        let new_stream: Stream<PaymentCoin> = Stream<PaymentCoin> {
            id: object::new(ctx),
            sender, 
            duration_in_seconds,
            last_timestamp_claimed_seconds: clock::timestamp_ms(clock) / 1000,
            amount: coin::into_balance<PaymentCoin>(payment)
        };

        event::emit(StreamCreatedEvent {
            stream_id: object::id(&new_stream), 
            sender, 
            receiver, 
            duration_in_seconds, 
            amount: coin_value
        ...
        transfer::transfer(new_stream, receiver);
    }
    ...
    */
    public fun claim_stream<PaymentCoin>(
        ...
        let stream_id: ID = object::uid_to_inner(&stream.id);

        let current_timestamp_seconds = clock::timestamp_ms(clock) / 1000;
        let time_passed_seconds = current_timestamp_seconds - stream.last_timestamp_claimed_seconds;

        let coin_to_receiver: Coin<PaymentCoin> = calculate_receiver_amount(&mut stream, clock, ctx);

        if(time_passed_seconds < stream.duration_in_seconds) {
            *&mut stream.last_timestamp_claimed_seconds = current_timestamp_seconds;
            *&mut stream.duration_in_seconds = *&stream.duration_in_seconds - time_passed_seconds;

            transfer::transfer(stream, tx_context::sender(ctx));
        } else {
            delete_stream_with_transfer(stream, ctx);
        };

        event::emit(StreamClaimedEvent {
            stream_id,
            receiver: tx_context::sender(ctx), 
            amount: coin::value<PaymentCoin>(&coin_to_receiver)
        });
    ...
    */
    public fun close_stream<PaymentCoin>(
        ...
        let stream_id: ID = object::uid_to_inner(&stream.id);
        let sender: address = *&stream.sender;

        let coin_to_receiver: Coin<PaymentCoin> = calculate_receiver_amount(&mut stream, clock, ctx);
        let amount_to_sender: u64 = delete_stream_with_transfer<PaymentCoin>(stream, ctx);

        event::emit(StreamClosedEvent {
            stream_id,
            receiver: tx_context::sender(ctx), 
            sender: sender, 
            amount_to_receiver: coin::value<PaymentCoin>(&coin_to_receiver),
            amount_to_sender
        });
    ...
    //==============================================================================================
    fun delete_stream_with_transfer<PaymentCoin>(stream: Stream<PaymentCoin>, ctx: &mut TxContext): u64 {
        let Stream<PaymentCoin> {id, sender, duration_in_seconds: _, last_timestamp_claimed_seconds: _, amount} = stream;

        let coin_to_sender: Coin<PaymentCoin> = coin::from_balance(amount, ctx);
        let amount_to_sender: u64 = coin::value(&coin_to_sender);

        if(amount_to_sender > 0) {
            transfer::public_transfer(coin_to_sender, sender);
        } else {
            coin::destroy_zero<PaymentCoin>(coin_to_sender);
        };
    ...
    fun calculate_receiver_amount<PaymentCoin>(
        stream: &mut Stream<PaymentCoin>, 
        clock: &Clock,
        ctx: &mut TxContext
    ): Coin<PaymentCoin> {
        let balance_amount: u64 = balance::value<PaymentCoin>(&stream.amount);

        let current_timestamp: u64 = clock::timestamp_ms(clock) / 1000;
        let time_passed = current_timestamp - *&stream.last_timestamp_claimed_seconds;

        let amount_to_receiver: u64 = 
        if(time_passed < *&stream.duration_in_seconds) 
            time_passed * balance_amount / *&stream.duration_in_seconds
        else 
            balance_amount;

        coin::take<PaymentCoin>(&mut stream.amount, amount_to_receiver, ctx)
    }
    //==============================================================================================
