...
module lp_account::liquidity_pool {
    ...
    */
    fun init_module(admin: &signer) {
        resource_account::create_resource_account(admin, SEED, x"");

        move_to(admin, State {
            signer_cap: resource_account::retrieve_resource_account_cap(admin, @overmind),
    ...
    */  
    public entry fun create_liquidity_pool<CoinA, CoinB>() acquires State {
        ...
        compare_coin_names<CoinA, CoinB>();

        let(lp_name, lp_symbol) = generate_lp_token_params(coin::symbol<CoinA>(), coin::symbol<CoinB>());

        let state: &mut State = borrow_global_mut<State>(@lp_account);
        let resource_signer: &signer = &account::create_signer_with_capability(&state.signer_cap);

        let(burn_cap, freeze_cap, mint_cap) = coin::initialize<LPCoin<CoinA, CoinB>>(resource_signer, lp_name, lp_symbol, 8, true);
        coin::destroy_freeze_cap<LPCoin<CoinA, CoinB>>(freeze_cap);

        coin::register<LPCoin<CoinA, CoinB>>(resource_signer);

        move_to(resource_signer, LiquidityPool<CoinA, CoinB> {
        ...
        event::emit_event<CreateLiquidityPoolEvent>(&mut state.create_liquidity_pool_events, CreateLiquidityPoolEvent {
    ...
    */
    public fun supply_liquidity<CoinA, CoinB>(
        ...
        let amount_coin_a: u128 = (coin::value<CoinA>(&coin_a) as u128);
        let amount_coin_b: u128 = (coin::value<CoinB>(&coin_b) as u128);
        assert!(amount_coin_a > 0 || amount_coin_b > 0, ECodeForAllErrors);

        let pool: &mut LiquidityPool<CoinA, CoinB> = borrow_global_mut<LiquidityPool<CoinA, CoinB>>(@lp_account); 

        let amount_coin_a_reserve: u128 = (coin::value<CoinA>(&pool.coin_a_reserve) as u128);
        let amount_coin_b_reserve: u128 = (coin::value<CoinB>(&pool.coin_b_reserve) as u128);

        let lp_tokens_to_mint: u128;
        let lp_supply: u128 = option::extract<u128>(&mut coin::supply<LPCoin<CoinA, CoinB>>());
        
        if(lp_supply > 0) {
            lp_tokens_to_mint = math128::min(amount_coin_a * lp_supply / amount_coin_a_reserve, amount_coin_b * lp_supply / amount_coin_b_reserve);
        } else {
            lp_tokens_to_mint = math128::sqrt(amount_coin_a * amount_coin_b);
            assert!(lp_tokens_to_mint > 1000, ECodeForAllErrors);

            lp_tokens_to_mint = lp_tokens_to_mint - 1000;
            coin::deposit<LPCoin<CoinA, CoinB>>(@lp_account, coin::mint<LPCoin<CoinA, CoinB>>(1000, &pool.lp_coin_mint_cap));
        };
        ...
        coin::mint<LPCoin<CoinA, CoinB>>((lp_tokens_to_mint as u64), &pool.lp_coin_mint_cap)
    }
    ...
    */
    public fun remove_liquidity<CoinA, CoinB>(
        ...
        let lp_amount = coin::value<LPCoin<CoinA, CoinB>>(&lp_coins_to_redeem);
        ...
        let amount_coin_a_reserve: u64 = coin::value<CoinA>(&pool.coin_a_reserve);
        let amount_coin_b_reserve: u64 = coin::value<CoinB>(&pool.coin_b_reserve);

        let amount_a_out: u64 = ((lp_amount * amount_coin_a_reserve as u128) / lp_supply as u64);
        let amount_b_out: u64 = ((lp_amount * amount_coin_b_reserve as u128) / lp_supply as u64);

        coin::burn<LPCoin<CoinA, CoinB>>(lp_coins_to_redeem, &pool.lp_coin_burn_cap);

        let coin_a_out: Coin<CoinA> = coin::extract<CoinA>(&mut pool.coin_a_reserve, amount_a_out);
        let coin_b_out: Coin<CoinB> = coin::extract<CoinB>(&mut pool.coin_b_reserve, amount_b_out);
    ...
    */
    public fun swap<CoinA, CoinB>(
        ...
        let amount_coin_a_in: u64 = coin::value<CoinA>(&coin_a_in);
        let amount_coin_b_in: u64 = coin::value<CoinB>(&coin_b_in);
        ...
        let const_k_before: u128 = (amount_coin_a_reserve * amount_coin_b_reserve as u128);
        let const_k_after: u128 = (amount_coin_a_reserve + amount_coin_a_in - amount_coin_a_out as u128) * 
        (amount_coin_b_reserve + amount_coin_b_in - amount_coin_b_out as u128);
        assert!(const_k_after >= const_k_before, ECodeForAllErrors);

        coin::merge<CoinA>(&mut pool.coin_a_reserve, coin_a_in);
        coin::merge<CoinB>(&mut pool.coin_b_reserve, coin_b_in);

        let coin_a_out: Coin<CoinA> = coin::extract<CoinA>(&mut pool.coin_a_reserve, amount_coin_a_out);
        let coin_b_out: Coin<CoinB> = coin::extract<CoinB>(&mut pool.coin_b_reserve, amount_coin_b_out);
    ...
    // Helper functions
    //==============================================================================================
    fun compare_coin_names<CoinA, CoinB>() {
        let coin_a_name: String = type_info::type_name<CoinA>();
        let coin_b_name: String = type_info::type_name<CoinB>();
        let result: &Result = &comparator::compare<String>(&coin_a_name, &coin_b_name);

        assert!(!comparator::is_equal(result), ECodeForAllErrors);
        assert!(comparator::is_smaller_than(result), ECodeForAllErrors);
    }

    fun generate_lp_token_params(coin_a_symbol: String, coin_b_symbol: String): (String, String) {
        let lp_coin_name = string_utils::to_string(&coin_a_symbol);
        string::append_utf8(&mut lp_coin_name, b"-");
        string::append(&mut lp_coin_name, string_utils::to_string(&coin_b_symbol));
        string::append_utf8(&mut lp_coin_name, b" LP token");

        let lp_coin_symbol: String = 
            if(string::length(&coin_a_symbol) > 4) 
                string::sub_string(&coin_a_symbol, 0, 4)
            else
                coin_a_symbol;

        string::append_utf8(&mut lp_coin_symbol, b"-");

        string::append(&mut lp_coin_symbol, 
            if(string::length(&coin_b_symbol) > 4)
                string::sub_string(&coin_b_symbol, 0, 4)
            else
                coin_b_symbol
        );

        (lp_coin_name, lp_coin_symbol)
    }
    //==============================================================================================
