...
module overmind::NonFungibleToken {
    ...
    */
    ...
    public fun mint_nft(
        ...
        assert!(coin::value(payment_coin) >= ONE_SUI, EInsufficientPayment);

        let balance: &mut Balance<SUI> = coin::balance_mut(payment_coin);
        let coin_to_send: Coin<SUI> = coin::take(balance, ONE_SUI, ctx);

        coin::put<SUI>(&mut minter_cap.sales, coin_to_send);
        ...
        transfer::transfer<NonFungibleToken>(new_nft, recipient);
    }
    ...
    public fun combine_nfts(
        ...
        let combined_nft = NonFungibleToken{
            id: object::new(ctx),
            name: create_combined_name(&nft1, &nft2),
            description: create_combined_description(&nft1, &nft2),
        ...
        destroy_nft_for_combaning(nft1);
        destroy_nft_for_combaning(nft2);
    ...
    public fun withdraw_sales(
        ...
        let balance: u64 = balance::value(&minter_cap.sales);

        event::emit(SalesWithdrawn {
            amount: balance
        });

        coin::take(&mut minter_cap.sales, balance, ctx)
    }
    ...
    //==============================================================================================
    fun create_combined_description(nft1: &NonFungibleToken, nft2: &NonFungibleToken): String {
        let description: String = string::utf8(b"Combined NFT of ");
        string::append(&mut description, name(nft1));
        string::append_utf8(&mut description, b" and ");
        string::append(&mut description, name(nft2));
        description
    }

    fun create_combined_name(nft1: &NonFungibleToken, nft2: &NonFungibleToken): String {
        let name: String = name(nft1);
        string::append_utf8(&mut name, b" + ");
        string::append(&mut name, name(nft2));
        name
    }

    fun destroy_nft_for_combaning(nft: NonFungibleToken) {
        let NonFungibleToken {id, name: _, description: _, image: _} = nft;
        object::delete(id);
    }
    //==============================================================================================
